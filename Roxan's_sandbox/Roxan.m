...
syms theta1_sym theta2_sym theta3_sym
% Initialisation
...
%%%%%%%%
X_end_effector = input('What is the X coordinate of the end-effector w.r.t the wrist? ');
Y_end_effector = input('What is the Y coordinate of the end-effector w.r.t the wrist? ');
...
P_end_effector = T_total(1:3, 4) + [X_end_effector;Y_end_effector;0];
...
% r = randi([1, 2]); %Taking a random configuration
% 
% Jacobian = jacobian(P_end_effector(1:2), [theta1_sym, theta2_sym, theta3_sym]);
...
% % Forward kinematics implementation of the robot
% nb_of_joints = input('Enter the number of joints of the robot: ');
% joints = sym(zeros(nb_of_joints + 1, 4));
% joints(1, :) = sym([0, 0, 0, 0]);
% T_matrixes = sym(zeros(4, 4, nb_of_joints));
% 
% for i = 2:nb_of_joints + 1
%     question_type = sprintf('Is joint n°%d revolute or prismatic? [R/P]: ', i-1);
%     type_of_joint = input(question_type, 's');
...
%         L = input(question_length);
% 
%         joints(i, :) = sym([0, L, offset, 0]);
% 
%     elseif type_of_joint == 'R'
%         question_theta = sprintf('What is the value of the angle between frame %d and frame %d? ', i-1, i-2);
%         theta = input(question_theta);
%         theta = deg2rad(sym(theta));
%         question_length = sprintf('What is the length of the arm between frame %d and frame %d? ', i-1, i-2);
%         L = input(question_length);
% 
%         joints(i, :) = sym([0, L, 0, theta]);
% 
%     else
%         disp('Invalid input. Please enter R for revolute or P for prismatic.');
% 
...
% end
% 
% X_end_effector = sym(input('What is the X coordinate of the end-effector w.r.t the wrist? '));
% Y_end_effector = sym(input('What is the Y coordinate of the end-effector w.r.t the wrist? '));
% 
% T_total = eye(4, 'sym');
% 
% for i = 1:nb_of_joints
...
% 
% % % Implementation of Jacobian Matrices
% % Jacobian = jacobian(P_end_effector(1:2), [theta1, theta2, theta3]);
% % 
